...
- 时间复杂度为O(n^2)
- **冒泡排序**：除非手边没有算法书可参照，一般情况很少用冒泡排序算法；当数据量很小的时候它会有价值外，其余都不建议使用。
- **选择排序**：虽然把交换次数降到了最低，但比较的次数仍比较大。当数据量很小，并且交换数据相对比较数据更加耗时的情况下，可以使用选择排序。
- **插入排序**：当数据量比较小或基本有序时，插入排序算法是三种排序中最好的选择。
